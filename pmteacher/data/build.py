...
from pmteacher.data.common import (
...
def build_detection_semisup_train_loader_two_crops_multi2(cfg, mapper=None, data_val = 0, super_only = True):
    if cfg.DATASETS.CROSS_DATASET:  # cross-dataset (e.g., coco-additional)
        if (super_only == True):
            label_dicts = get_detection_dataset_dicts(
                cfg.DATASETS.TRAIN_LABEL[data_val],
                filter_empty=cfg.DATALOADER.FILTER_EMPTY_ANNOTATIONS,
                min_keypoints=cfg.MODEL.ROI_KEYPOINT_HEAD.MIN_KEYPOINTS_PER_IMAGE
                if cfg.MODEL.KEYPOINT_ON
                else 0,
...
def build_detection_semisup_train_loader_two_crops_multi(cfg, mapper=None):
    if cfg.DATASETS.CROSS_DATASET:  # cross-dataset (e.g., coco-additional)
        label_dicts = []
        #print(cfg.DATASETS.TRAIN_LABEL)
        for i, label in enumerate(cfg.DATASETS.TRAIN_LABEL):
            print(label)
            label_dicts.append(get_detection_dataset_dicts(
                label,
                filter_empty=cfg.DATALOADER.FILTER_EMPTY_ANNOTATIONS,
                min_keypoints=cfg.MODEL.ROI_KEYPOINT_HEAD.MIN_KEYPOINTS_PER_IMAGE
                if cfg.MODEL.KEYPOINT_ON
            ...
            print("hi")
        #print(cfg.DATASETS.TRAIN_UNLABEL)
        unlabel_dicts = get_detection_dataset_dicts(
            cfg.DATASETS.TRAIN_UNLABEL,
            filter_empty=False,
            min_keypoints=cfg.MODEL.ROI_KEYPOINT_HEAD.MIN_KEYPOINTS_PER_IMAGE
    ...
    label_dataset = []
    for i, label in enumerate(label_dicts):
        label_dataset.append(DatasetFromList(label_dicts[i], copy=False))
    # exclude the labeled set from unlabeled dataset
    unlabel_dataset = DatasetFromList(unlabel_dicts, copy=False)
    # include the labeled set in unlabel dataset
    # unlabel_dataset = DatasetFromList(dataset_dicts, copy=False)
    ...
    for i, label in enumerate(label_dataset):
        label_dataset[i] = MapDataset(label_dataset[i], mapper)
    unlabel_dataset = MapDataset(unlabel_dataset, mapper)
    ...
    if sampler_name == "TrainingSampler":
        label_sampler = []
        for i, label in enumerate(label_dataset):
            label_sampler.append(TrainingSampler(len(label)))
        unlabel_sampler = TrainingSampler(len(unlabel_dataset))
    elif sampler_name == "RepeatFactorTrainingSampler":
        raise NotImplementedError("{} not yet supported.".format(sampler_name))
    else:
    ...
    for i in range(len(label_dataset)):
        to_return.append(build_semisup_batch_data_loader_two_crop(
        (label_dataset[i], unlabel_dataset),
        (label_sampler[i], unlabel_sampler),
        cfg.SOLVER.IMG_PER_BATCH_LABEL,
        cfg.SOLVER.IMG_PER_BATCH_UNLABEL,
        aspect_ratio_grouping=cfg.DATALOADER.ASPECT_RATIO_GROUPING,
        num_workers=cfg.DATALOADER.NUM_WORKERS,
