...
from pmteacher.modeling.roi_heads.fast_rcnn import FastRCNNFocaltLossOutputLayers
...
class StandardROIHeadsPseudoLab(StandardROIHeads):
    ...
    def forward(
        ...
        if (branch.startswith("prototype_all")):
            box_features =  self._forward_box(
                features, proposals, compute_loss, compute_val_loss, branch
            )
            return box_features

        if (self.training and compute_loss) or compute_val_loss:
            losses, prediction, box_features = self._forward_box(
                features, proposals, compute_loss, compute_val_loss, branch
            )
    ...
    def label_and_sample_proposals(
        ...
        if (self.proposal_append_gt and not (branch.startswith("prototype"))):
